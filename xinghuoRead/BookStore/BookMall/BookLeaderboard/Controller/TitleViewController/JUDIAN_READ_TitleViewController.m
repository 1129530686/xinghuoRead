//
//  JUDIAN_READ_TitleViewController.m
//

#import "JUDIAN_READ_TitleViewController.h"
#import "JUDIAN_READ_TitleLabel.h"
#import "JUDIAN_READ_TitleHeader.h"
#import "UIView+JUDIAN_READ_Frame.h"
#import "JUDIAN_READ_TitleFlowLayout.h"

#define CONTENTCELL_ID  @"CONTENTCELL"

@interface JUDIAN_READ_TitleViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
}
...
/**
 颜色渐变样式
 */
@property (nonatomic, assign) TitleColorGradientStyle titleColorGradientStyle;
...
@implementation JUDIAN_READ_TitleViewController
...
- (UIView *)underLine {
    ...
    if (!_underLine) {
        ...
        underLineView.backgroundColor = _underLineColor ? _underLineColor : READER_SETTING_PANEL_BUTTON_CLICKED_COLOR;
        //[self.titleScrollView addSubview:underLineView];
        [self.titleScrollView insertSubview:underLineView aboveSubview:self.bottomLineView];
...
- (UIScrollView *)titleScrollView {
    if (!_titleScrollView) {
        ...
        UIView* lineView = self.bottomLineView;
...
- (UIScrollView *)contentScrollView
{
    if (!_contentScrollView) {
        ...
        // 创建布局
        JUDIAN_READ_TitleFlowLayout *layout = [[JUDIAN_READ_TitleFlowLayout alloc] init];
        ...
        // 注册cell
        [_contentScrollView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:CONTENTCELL_ID];
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void(^)(TitleColorGradientStyle *titleColorGradientStyle,UIColor **norColor,UIColor **selColor))titleGradientBlock;
{
    ...
    if (_titleColorGradientStyle == kTitleColorGradientStyleFill && _titleWidth > 0) {
...
- (void)setUpUnderLineEffect:(void(^)(BOOL *isUnderLineDelayScroll,CGFloat *underLineH,UIColor **underLineColor,BOOL *isUnderLineEqualTitleWidth))underLineBlock
{
    ...
    _isShowUnderLine = ![self isMiddleLineStyle];
...
- (void)viewDidLayoutSubviews {
    ...
    if (_isInitial == NO) {
        ...
        CGRect titleFrame = [self getTitleFrame];
        CGFloat titleY = 0;
        // 是否占据全屏
        if (_isfullScreen) {
            
            // 整体contentView尺寸
            self.contentView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - bottomOffset);
            
            // 顶部标题View尺寸
            self.titleScrollView.frame = titleFrame;
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, titleY, SCREEN_WIDTH, SCREEN_HEIGHT - titleY - bottomOffset - navigationHeight);
            if ([self isMiddleLineStyle]) {
                self.contentView.frame = CGRectMake(0, titleY, SCREEN_WIDTH, SCREEN_HEIGHT);
            }
        }
        ...
        self.contentScrollView.frame = CGRectMake(0, contentY, SCREEN_WIDTH, contentH);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!_isInitial) {
        ...
        if (_titleColorGradientStyle == kTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要这样
...
- (CGRect)getTitleFrame {
    return CGRectMake(0, 0, SCREEN_WIDTH, self.titleHeight);
}
...
- (void)setUpAllTitle {
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[JUDIAN_READ_TitleLabel alloc] init];
        label.tag = i;
        // 设置按钮的文字颜色
        ...
        if ([self isMiddleLineStyle]) {
            ...
            if (_titleColorGradientStyle == kTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要
                labelW = [self.titleWidths[i] floatValue];
                // 设置按钮位置
                UILabel *lastLabel = [self.titleLabels lastObject];
                labelX = _titleMargin + CGRectGetMaxX(lastLabel.frame);
    ...
    if ([self isMiddleLineStyle]) {
        ...
        verticalLineView.backgroundColor = RGB(0xcc, 0xcc, 0xcc);
        [_titleScrollView addSubview:verticalLineView];
        UILabel* firstTitleLabel = self.titleLabels[0];
        CGFloat xPosition = CGRectGetMaxX(firstTitleLabel.frame);
        xPosition += _titleMargin / 2.0f;
        verticalLineView.frame = CGRectMake(xPosition, 0, 1, 15);
        verticalLineView.centerY = firstTitleLabel.centerY;
    }
    ...
    _contentScrollView.contentSize = CGSizeMake(count * SCREEN_WIDTH, 0);
    
}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(JUDIAN_READ_TitleLabel *)rightLabel leftLabel:(JUDIAN_READ_TitleLabel *)leftLabel
{
    ...
    // RGB渐变
    if (_titleColorGradientStyle == kTitleColorGradientStyleRGB) {
    ...
    // 填充渐变
    if (_titleColorGradientStyle == kTitleColorGradientStyleFill) {
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / SCREEN_WIDTH;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / SCREEN_WIDTH;
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        [self clickWhichIndex:i];
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:kClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:kRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label {
    ...
    for (JUDIAN_READ_TitleLabel *labelView in self.titleLabels) {
        ...
        if (_isShowTitleGradient && _titleColorGradientStyle == kTitleColorGradientStyleFill) {
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    if (self.underLine.x == 0) {
        if (_isUnderLineEqualTitleWidth) {
            self.underLine.width = titleBounds.size.width / 2;
        } else {
            self.underLine.width = label.width / 2;
        }
        
        self.underLine.width += 2;
    ...
    WeakSelf(that);
    [UIView animateWithDuration:0.25 animations:^{
        if (that.isUnderLineEqualTitleWidth) {
            self.underLine.width = titleBounds.size.width / 2;
        } else {
            self.underLine.width = label.width / 2;
        }
...
{
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CONTENTCELL_ID forIndexPath:indexPath];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger screenWInt = SCREEN_WIDTH;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 左边按钮
    JUDIAN_READ_TitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    JUDIAN_READ_TitleLabel *rightLabel = nil;
    ...
    // 设置下标偏移
    if (!_isDelayScroll) { // 延迟滚动，不需要移动下标
    ...
    [self scrollUnderLine:scrollView];
    
}


- (void)scrollUnderLine:(UIScrollView *)scr{
    ...
    NSInteger segmentWidth = _segmentWidth;
    
    JUDIAN_READ_TitleLabel *rightLabel = self.titleLabels[self.selectItem];
    CGRect titleBoundsR = [rightLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
    
    NSInteger titleWidth = titleBoundsR.size.width / 2 + 2;
    
    CGFloat x = segmentWidth / SCREEN_WIDTH * changeX;
    CGFloat positionX = segmentWidth / SCREEN_WIDTH * positionChangeX;
    
    if (x > 0) {
        self.underLine.frame = CGRectMake(segmentWidth * self.selectItem + (segmentWidth - titleWidth) / 2.0 , _titleHeight - 2, titleWidth + positionX, 2);
    }else if(x < 0) {
        self.underLine.frame = CGRectMake(segmentWidth * self.selectItem + (segmentWidth - titleWidth) / 2.0 + x, _titleHeight - 2, titleWidth + positionX, 2);
    }
