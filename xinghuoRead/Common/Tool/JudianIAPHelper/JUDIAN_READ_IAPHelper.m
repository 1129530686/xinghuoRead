...
#import "JUDIAN_READ_KeychainUtils.h"

#if ! __has_feature(objc_arc)
#error You need to either convert your project to ARC or add the -fobjc-arc compiler flag to JUDIAN_READ_IAPHelper.m.
#endif


@interface JUDIAN_READ_IAPHelper()
...
@implementation JUDIAN_READ_IAPHelper
...
- (id)initWithProductIdentifiers:(NSSet *)productIdentifiers {
    if ((self = [super init])) {
        ...
        for (NSString * productIdentifier in _productIdentifiers) {
            ...
            NSString* password = [JUDIAN_READ_KeychainUtils getPasswordForUsername:productIdentifier andServiceName:@"IAPHelper" error:nil];
...
-(BOOL)isPurchasedProductsIdentifier:(NSString*)productID
{
    ...
    NSString* password = [JUDIAN_READ_KeychainUtils getPasswordForUsername:productID andServiceName:@"IAPHelper" error:nil];
    if([password isEqualToString:@"YES"])
    {
        productPurchased = YES;
    }
...
- (void)provideContentWithTransaction:(SKPaymentTransaction *)transaction {
    ...
    if (productIdentifier) {
        [JUDIAN_READ_KeychainUtils storeUsername:productIdentifier andPassword:@"YES" forServiceName:@"IAPHelper" updateExisting:YES error:nil];
...
- (void)clearSavedPurchasedProductByID:(NSString*)productIdentifier {
    ...
    [JUDIAN_READ_KeychainUtils deleteItemForUsername:productIdentifier andServiceName:@"IAPHelper" error:nil];
